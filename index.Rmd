...
```{r warning=FALSE, echo = F, results = 'asis', fig.width=11, fig.height=7}
dowdata <- read.csv("HistoricalPrices.csv")
dowdata$Date <- as.Date(dowdata$Date, "%m/%d/%y")
ggplot() +
      xlab("Date") + 
      ylab("Index") +
      ggtitle("DJIA from Feb 1985 to Sep 2015") +
      geom_line(data=dowdata, aes(x=Date, y=Close), colour="blue")
